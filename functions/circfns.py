...
# operations required to write out matrix representations of different Toffolis on full circuit
...
# Different Toffolis with different controls

# T1 conditional on 1,1 - acts on qubits 1,2 and flips 8
U_T11 = tensor(zero, tensor(zero, I_6)) + \
        tensor(zero, tensor(one, I_6)) + \
        tensor(one, tensor(zero, I_6)) + \
        tensor(one, tensor(one, tensor(I_5, X)))


# T2 conditional on 1,0 - acts on qubits 1,2 and flips 8
U_T21 = tensor(zero,tensor(zero,I_6)) + \
        tensor(zero,tensor(one, I_6)) + \
        tensor(one,tensor(zero,tensor(I_5, X))) + \
        tensor(one,tensor(one, I_6))
# T3 conditional on 0,1 - acts on qubits 1,2 and flips 8
U_T31 = tensor(zero,tensor(zero, I_6)) + \
        tensor(zero,tensor(one,tensor(I_5, X))) + \
        tensor(one,tensor(zero, I_6)) + \
        tensor(one,tensor(one,tensor(I_5,X)))
# T4 conditional on 0,0 - acts on qubits 1,2 and flips 8
U_T41 = tensor(zero,tensor(zero,tensor(I_5, X))) + \
        tensor(zero,tensor(one, I_6)) + \
        tensor(one,tensor(zero, I_6)) + \
        tensor(one,tensor(one, I_6))

# T1 conditional on 1,1 - acts on qubits 3,8 and flips 7
U_T12 = tensor(I_2, tensor(zero, tensor(I_4, zero))) + \
        tensor(I_2, tensor(zero, tensor(I_4, one))) + \
        tensor(I_2, tensor(one, tensor(I_4, zero))) + \
        tensor(I_2, tensor(one, tensor(I_3, tensor(X, one))))
# T3 conditional on 0,1 - acts on qubits 3,8 and flips 7
U_T32 = tensor(I_2, tensor(zero, tensor(I_4, zero))) + \
        tensor(I_2, tensor(zero, tensor(I_3, tensor(X, one)))) + \
        tensor(I_2, tensor(one, tensor(I_4, zero))) + \
        tensor(I_2, tensor(one, tensor(I_4, one)))
